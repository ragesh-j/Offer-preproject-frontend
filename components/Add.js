...
function Add() {
  ...
  useEffect(() => {
    if (data.offerimage) {
      ...
      formdata.append("upload_preset","insta_clone_ragesh")
      formdata.append("cloud_name","dql4bctke")
      fetch("https://api.cloudinary.com/v1_1/dql4bctke/image/upload", {
        method: "POST",
        body: formdata,
      })
        .then((res) => res.json())
  ...
  function add(e) {
    ...
    formData.append("offertitle", data.offertitle);
      fetch("http://localhost:8080/offer/add", {
        method: "POST",
        headers: {
          Authorization: token,
        },
